...
db1 = mysql.connector.connect(option_files="my.cnf", autocommit=True)

db2 = mysql.connector.connect(option_files="my.cnf", autocommit=True)
...
VALUES (DEFAULT, 'Camelot', 'GBR', 'King Arthur County',2000)"""
)

print("\nin_transaction = {0}".format(db1.in_transaction))

id = cursor1.lastrowid
sql = """SELECT * FROM world.city WHERE id = {0}""".format(id)
cursor1.execute(sql)
cursor2.execute(sql)
